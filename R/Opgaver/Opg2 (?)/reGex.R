...
full_links_df <- data.frame(links = as.character())
for (i in 1:length(full_links)) {
  tmp_link <- data.frame(links=full_links[[i]])
  full_links_df <- rbind(full_links_df, tmp_link)
}

full_links_df <- full_links_df %>% 
  rowwise() %>% 
  mutate(sagnr = str_extract(links, "(?<=sagsnr=)[0-9]+"))
